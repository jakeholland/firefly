...
- **2026-08-24, S16 slice c1 (input ownership — supersedes the Input line's "swipe left/right = face nav (owned by S06's shell `scr_nav.c`)" and slice (d)'s "FLARE button hook (fires S10 callback)").** Face navigation is owned by `app/ff_route.c` + `ff_shell_intent` (S16); `scr_nav.c` is chrome only. Interactive controls hand a semantic intent up through the emit seam (`app/include/ff_intent.h`) and the shell decides — no screen file decides which face is active, ever. As of S16c1 the long-press-Settings hook emits `FF_INTENT_OPEN_SETTINGS` (which the shell deliberately rejects until S11 slice b ships a Settings renderer — see `ff_shell.c`'s judgment-call note). Two residuals land with their own S16 slices, recorded so this entry doesn't overclaim: the FLARE button still calls `ff_flare_send_begin` directly until slice c2 converts it to `FF_INTENT_FLARE_START`, and the tileview still performs the physical swipe gesture itself until slice c3 disables `LV_DIR_HOR` scrolling and routes the gesture through `FF_INTENT_SWIPE` (S16 AC2 cannot fully hold until it does). Criterion 5's "swipe changes face" is then a statement about `ff_route_swipe` via the seam, not about `lv_tileview`.
