...
- **2026-08-25, slice b implementation — interpretation calls (AGENTS.md: "note the interpretation in the PR body" — recorded here too, per this repo's own convention).**

  - **`FF_HIT_MIN_GAP_PX` = 8px, edge-to-edge, not centre-to-centre.** The AC2 text ("centre-to-centre or edge-to-edge — justify") left both open. Edge-to-edge was chosen because it measures the actual physical dead-space a thumb needs to land inside of to miss BOTH controls — independent of either control's own size — whereas centre-to-centre is biased by control size (a huge chip next to a tiny icon reads as "far apart" by centre distance while their edges nearly touch). 8px (not `FF_SETTINGS_CHIP_GAP`'s 24px) was chosen as the FLOOR — the minimum below which a gap is a genuine mis-tap trap — citing Google's Material Design accessibility guidance (>= 8dp between adjacent touch targets), deliberately smaller than the 24px PR #68 chose as a GENEROUS separation for one specific case (two chips on the same row), not re-derived as the universal minimum. Full rationale lives with the constant, `ff_theme.h`.

  - **Adjacency scope: sibling clickable elements sharing an immediate LVGL parent, excluding pairs that are the SAME logical control.** A naive all-pairs sweep would flag every real control against `scr_nav.c`'s long-press region and `scr_map.c`'s tap-anywhere-back puck (both CLICKABLE, sized to the whole puck, with every real control nested somewhere underneath) — not a real mis-tap risk, since a touch always resolves to the deepest clickable object, never ambiguously to an ancestor and descendant at once. Sibling-scoping sidesteps that without a special-cased exclusion list. Separately, `scr_settings.c`'s WATER NUDGE/QUIET HOURS rows wire a dim label and its own value chip to the IDENTICAL callback+argument (one setting, two hit-rects) — detected generically via matching registered click-callback function pointer + `user_data` (LVGL introspection: `lv_obj_get_event_dsc`/`lv_event_dsc_get_cb`/`_get_user_data`), not a per-screen exception list naming those two rows, so a future composite control gets the same treatment without a code change to the sweep. Full rationale: `test_face_hit_targets.c`'s header comment.

  - **Real violation found and fixed: Compose's T9 keypad (`FF_COMPOSE_KEY_GAP`, 6px -> 10px).** AC1 already passed clean (0 violations, 253 clickable elements, 44 fixtures) before this slice — the sweep's existing size floor had no real gaps to close. AC2's new adjacency pass found exactly one real violation class: every adjacent pair of T9 grid keys (both within a row and between rows, since one constant governs both) measured 6.0-6.7px apart, under the 8px floor — 72 violations across the four `compose_*.json` fixtures, zero elsewhere (Settings' double-chip rows, already fixed by PR #68 at 24px, and its 10px inter-row gap, and Signals' OMW/5 MIN/PULSE row at 8px, all cleared the floor as built). Fixed by widening `FF_COMPOSE_KEY_GAP` to 10px; every key's own width/height stays comfortably clear of the 44px floor at the new gap (verified against the real rendered click-area: row0 is 132x46, row1 122x46, row2 100x46, and the narrowest case, the bottom DEL/0/SEND row, is 50x46 — corrected here per PR #86 code review, which caught an earlier draft of this note quoting a stale 64x46 that didn't account for the grid's Y-cascade). This moved the four `compose_*.json` goldens (`compose_abc`/`compose_123`/`compose_sym`/`compose_mid` — the keypad grid shifts down slightly and keys narrow slightly to make room; no key goes off-glass or under the size floor); every other golden (40 of 44) stayed byte-identical. Bailey review requested per AGENTS.md (UI change).

- **2026-08-25, code review round 1 (PR #86) — one blocking real violation the fixtures didn't cover, a real coverage gap in the check itself, and two non-blocking follow-ups. The gate is only as good as its fixtures AND its scope — this round found both kinds of gap.**

  - **[Blocking, fixed] `scr_signals.c`'s feed-row gap (`FF_SIGNALS_ROW_GAP`, 6px) was the SAME mis-tap trap the T9 keypad had, live in production, and the committed fixtures never caught it.** Adjacent feed rows are clickable-sibling CANDIDATES whenever consecutive items are both RALLY kind (the only clickable kind — `signals_build_row`), but `signals_feed.json` carried exactly one RALLY item, so the AC2 sweep never exercised two-RALLY adjacency. Closed on BOTH halves the reviewer asked for: the constant (6px -> 10px, matching this codebase's other "modest gap" precedent, plus a `_Static_assert(FF_SIGNALS_ROW_GAP >= FF_HIT_MIN_GAP_PX, ...)` so a future regression fails the BUILD, not just the sweep) and a new fixture, `tests/fixtures/signals_two_rallies.json` (two adjacent RALLY items), so the gate now actually covers this case rather than merely happening to pass it. Mutation-verified on a fresh `rm -rf build/{app,targets}` rebuild (object hash confirmed changed): reverting the gap to 6px with the fixture present fails the sweep (`HIT-TARGETS-TOO-CLOSE [signals_two_rallies.json] ... gap=6.0px (floor 8px)`); the same mutation against the OLD `signals_feed.json` alone (one RALLY item) would have passed silently — exactly the coverage gap being closed. This moved `signals_feed.png`'s own golden (rows 2/3 shift down ~4px each for the wider inter-row gap; row 1/header/reply-row unaffected) and added `signals_two_rallies.png`; every other golden stayed byte-identical.

  - **[Should-fix, fixed] The adjacency check's scope was too narrow — "sibling clickable elements sharing an immediate LVGL parent" structurally could not see two clickable COUSINS (different parents, neither an ancestor of the other) no matter how close together they sat.** Reviewer's concrete instance: `scr_signals.c`'s header "+" button and the first feed row measure exactly 8px apart (the floor itself, zero slack) and share no parent (the "+" is a child of the face's own root container; feed rows are children of the scrollable `list` beneath it) — invisible to the old scope by construction, at any gap value. Resolved via option (a) from the reviewer's own menu — the robust global fix, not a documented limitation — a GLOBAL all-pairs sweep over every clickable element in the tree, with two exclusions replacing sibling-scoping's implicit protection: (1) ancestor/descendant (walking `lv_obj_get_parent`, not just immediate-parent equality, so it also catches a control nested several levels under an enclosing region), which is what makes `scr_nav.c`'s long-press region and `scr_map.c`'s tap-anywhere-back puck safe to compare against everything nested under them without a false "violation" against their own enclosing region; and (2) the SAME whole-puck-square exact-size match AC1's circle check already uses, needed as a SEPARATE exclusion because going global surfaced a real case Exclusion 1 alone didn't cover — `scr_nav.c`'s `tileview` (kept CLICKABLE deliberately, for its own gesture/long-press-fallback handling) sits as a SIBLING, not ancestor, of the flare sender overlay's CANCEL button, both direct children of `puck`, and measured a spurious 0.0px "gap" against it before this exclusion was added (not a real mis-tap risk — a touch anywhere on CANCEL's own more-specific rect still resolves to CANCEL first, per LVGL's deepest-hit-wins search, same reasoning Exclusion 1 relies on). Re-verified the +button/first-row cousins pair is now actually evaluated (not accidentally excluded): measured gap is exactly 8.0px, passing (not `< 8`) with zero margin — the check now genuinely SEES this pair, which is what "we enforce a tap-target floor" should mean, even though this particular instance doesn't currently need a layout fix.
    - **A second, related real finding surfaced only by going global**, fixed alongside: `scr_nav.c`'s three tileview tile CONTAINERS (`tile_radar`/`tile_now`/`tile_signals`) had always been left `CLICKABLE` by omission (`lv_tileview_add_tile` defaults it on, same class of bug `scr_signals.c`'s own header comment already documents fixing for its container primitives) — harmless on its own (no click handler was ever attached directly to a tile), but the two currently-scrolled-off-screen tiles sit at their native, un-scrolled tileview-grid positions (hundreds of px outside the window) and, once the whole-puck-size exemption stopped blanket-covering them for adjacency, showed as 58 spurious HIT-TARGETS-TOO-CLOSE findings between tiles that were never independently reachable by a real touch (only one tile is ever visible; a real finger cannot land at a coordinate the physical display doesn't have). Fixed by explicitly clearing `LV_OBJ_FLAG_CLICKABLE` on all three tiles — verified, not assumed, that long-press-to-Settings reachability survives via `test_scr_intent.c`'s existing `S16_c3_physical_long_press_on_empty_puck_space_reaches_open_settings`/`S16_c3_content_button_long_press_does_not_reach_open_settings` probes (both still pass: a touch over empty tile space now resolves one level up, at the still-CLICKABLE `tileview`, and bubbles to `puck` exactly as before — one hop shorter, same destination).

  - **[Non-blocking, fixed] The composite-control exclusion (Exclusion 1) had never been exercised — no committed fixture's PASS/FAIL depends on it, since every current composite pair sits at `FF_SETTINGS_CHIP_GAP` (24px), already well above the 8px floor whether or not the exclusion fires (confirmed: mutating the function to `return false;` unconditionally still swept every fixture at 0 violations).** No fixture CAN exercise it end-to-end by construction — the label/chip gap is a compile-time layout constant, never fixture data, so no `*.json` changes that geometry, and deliberately shrinking a real, reviewed 24px gap purely to manufacture a fixture-visible failure would trade a real UX margin for test coverage of a code path, backwards. Closed with a direct unit test instead (`S17b_AC2_composite_control_detection`, `test_face_hit_targets.c`) exercising `sweep_same_composite_control` against synthetic LVGL buttons: true only when BOTH the callback pointer and `user_data` match, false the moment either differs, false (never a false-positive) when either side has no registered callback at all.

  - **[Non-blocking, fixed] Documentation error, caught independently by both reviewers: this file's own AC2 note (and `scr_compose.c`'s matching comment) claimed the narrowest T9 row — bottom DEL/0/SEND — measures "64x46".** It's **50x46**, verified against the real rendered click-area (not hand math): row0 132x46, row1 122x46, row2 100x46, bottom 50x46. The stale number came from an earlier calculation that didn't account for the grid's Y-position cascade the `FF_COMPOSE_KEY_GAP` bump causes (later rows shift down as earlier rows' gaps grow, which also narrows the chord-derived margin at each row — a hand-math trap the file's own header comment already names for exactly this reason: "verified against a real headless render, not just arithmetic"). Both comments corrected; the number was descriptive documentation only — every key still clears the 44px floor regardless, so no behavior was ever affected.
