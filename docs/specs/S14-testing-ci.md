...
4. **E2E** (`tests/e2e/`, pytest): compose up meshtasticd → crew_sim scenarios → ctl-socket assertions + screenshots. Scenarios v1: `test_dana_walk` (position flows to radar view state), `test_flare_reaches_feed` (renamed 2026-09-02 from `test_pulse_feed`/`test_pulse_reaches_feed` — PULSE is retired, see docs/specs/S04-firefly-protocol.md's Amendments), `test_text_roundtrip`. Runs nightly + on `e2e` label (docker too slow for every PR). Only 1 of these 3 scenarios (`test_position_reaches_radar`) can actually pass today: the other two are `xfail(strict=False)`, not skipped — the pinned meshtasticd image only allows one connected TCP client at a time, so `crew_sim.py` triggering a send force-disconnects `ffsim`'s own connection and the packet isn't redelivered on reconnect. This is a documented meshtasticd limitation, not a flake — see `firmware/tests/e2e/test_scenarios.py`'s module docstring for the full root-cause writeup.
