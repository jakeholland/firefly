...
- **Same drift rule, new instance (A02 slice D, 2026-09-13):** the SHOW CODE face needs `CONFIG_LV_USE_QRCODE=y`. It is in `sdkconfig.defaults`, but a board `sdkconfig` generated before that entry existed still holds `# CONFIG_LV_USE_QRCODE is not set` and that value wins — verified on the bring-up board, where the build fails to compile `scr_settings.c`. Enable it in `idf.py menuconfig` (Component config → LVGL configuration → 3rd party libraries → QR code), or delete the key from the board's `sdkconfig` and rebuild. `scr_settings.c` has an `#error` that says this, so the failure names the config rather than LVGL.
...
| `diag` | DIAGNOSTICS: the SAME facts the Settings DIAGNOSTICS page shows — link, my position, mesh link-quality, wall clock, compass, device — as eight `dbg: diag ...` lines (Link split identity/counters, Mesh split roster-RF/airtime), plus (2026-09-16 S26f field fix) a ninth `dbg: diag wakes n=<count>; ...` line summarizing the last up to 8 light-sleep wake causes (both scheduled and forced by `sleep`) |
...
| `sleep` / `sleep <ms>` | 2026-09-16 S26f field fix: force ONE light-sleep cycle right now, ignoring the USB-connected inhibit (every wake source stays armed) — reports the wake cause, the touch-INT GPIO level sampled just before sleep and just after wake, and the elapsed time. `<ms>` (50-60000) overrides the default 1500ms timer-wake period. The reply may be lost (USB drops during the sleep itself) — reconnect and read `diag`'s `wakes` line if so. See `docs/specs/S26-device-lifecycle.md` slice (f)'s dated amendment for the full bug + bench evidence |
| `tpint` | 2026-09-16 S26f field fix: poll the touch-INT GPIO level for a fixed 5s window — tap the glass during that window to see whether the line moves |
