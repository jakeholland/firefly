/**
 * test_lineup_layout.c — assertion-level coverage for lineup_layout.c's pure
 ...
 * AC6 ("Goldens: lineup_live.json and lineup_tbd.json match"): these are the
...
/* ---------------------------------------------------------------------
 * lineup_layout_format_countdown — the spec's literal "IN N MIN" text.
 * ------------------------------------------------------------------- */
...
static void S07_AC6_countdown_format_matches_spec_example(void)
{
    ...
    char buf[16];
    lineup_layout_format_countdown(33, buf, sizeof(buf));
...
{
    char buf[16];
    lineup_layout_format_countdown(0, buf, sizeof(buf));
...
static void S07_AC6_countdown_format_clamps_negative_to_zero(void)
{
    ...
    char buf[16];
    lineup_layout_format_countdown(-5, buf, sizeof(buf));
    TEST_ASSERT_EQUAL_STRING("IN 0 MIN", buf);
}
...
{
    char buf[16];
    lineup_layout_format_countdown(1, buf, sizeof(buf));
    TEST_ASSERT_EQUAL_STRING("IN 1 MIN", buf);
}

/* PR #21 code review finding #6: the 59/60-minute boundary specifically.
 * lineup_layout_format_countdown has no hour-conversion branch — it's a flat
...
{
    char buf[16];
    lineup_layout_format_countdown(59, buf, sizeof(buf));
...
{
    char buf[16];
    lineup_layout_format_countdown(60, buf, sizeof(buf));
...
static void S07_AC6_countdown_format_null_out_is_a_noop(void)
{
    ...
    lineup_layout_format_countdown(10, NULL, 16);
    TEST_PASS();
}

/* ---------------------------------------------------------------------
 * lineup_layout_bar_fill_px — progress-bar pixel width from pct_done.
 * ------------------------------------------------------------------- */

static void S07_AC6_bar_fill_zero_pct_is_zero_width(void)
{
    TEST_ASSERT_EQUAL_INT32(0, lineup_layout_bar_fill_px(0, 200));
}

static void S07_AC6_bar_fill_hundred_pct_is_full_width(void)
{
    TEST_ASSERT_EQUAL_INT32(200, lineup_layout_bar_fill_px(100, 200));
}

static void S07_AC6_bar_fill_midpoint(void)
{
    /* Exact: 50% of 210 (LINEUP_LAYOUT_ROW_BAR_TRACK_W_PX) is 105 — no
     * rounding ambiguity to paper over. */
    TEST_ASSERT_EQUAL_INT32(105, lineup_layout_bar_fill_px(50, 210));
}
...
static void S07_AC6_bar_fill_clamps_over_100_pct(void)
{
    ...
    TEST_ASSERT_EQUAL_INT32(200, lineup_layout_bar_fill_px(255, 200));
}

static void S07_AC6_bar_fill_clamps_negative_track_width(void)
{
    TEST_ASSERT_EQUAL_INT32(0, lineup_layout_bar_fill_px(50, -10));
}

/* ---------------------------------------------------------------------
 * lineup_layout_chord_half_width_px — round-screen keep-out primitive.
 * ------------------------------------------------------------------- */

static void S07_AC6_chord_half_width_at_center_equals_radius(void)
{
    TEST_ASSERT_FLOAT_WITHIN(0.001f, 220.0f, lineup_layout_chord_half_width_px(0.0f));
}

static void S07_AC6_chord_half_width_at_top_edge_is_zero(void)
{
    TEST_ASSERT_FLOAT_WITHIN(0.001f, 0.0f, lineup_layout_chord_half_width_px(-220.0f));
}
...
static void S07_AC6_chord_half_width_beyond_radius_clamps_to_zero(void)
{
    ...
    float w = lineup_layout_chord_half_width_px(300.0f);
...
static void S07_AC6_chord_half_width_known_pythagorean_value(void)
{
    ...
    TEST_ASSERT_FLOAT_WITHIN(0.01f, 176.0f, lineup_layout_chord_half_width_px(132.0f));
}

static void S07_AC6_chord_half_width_symmetric_above_and_below_center(void)
{
    TEST_ASSERT_EQUAL_FLOAT(lineup_layout_chord_half_width_px(90.0f), lineup_layout_chord_half_width_px(-90.0f));
}
