...
static ff_fixture_result_t fx_parse_crew_page(fx_ctx_t const *c, int obj_i, ff_app_crew_page_t *cw)
{
    ...
    /* A02 slice D2 amendment (#47) — the SHOW CODE face's honest
     * PRECISION line. */
    if (fx_obj_get(c, obj_i, "precision_exact", &t)) cw->precision_exact = fx_bool(c, t, false);
...
int ff_fixture_dump_json(ff_app_state_t const *s, char *buf, size_t buf_sz)
{
    ...
    fw_raw(&w, s->settings.crew.precision_exact ? ",\"precision_exact\":true" : ",\"precision_exact\":false");
    fw_raw(&w, ",\"op\":\"");
