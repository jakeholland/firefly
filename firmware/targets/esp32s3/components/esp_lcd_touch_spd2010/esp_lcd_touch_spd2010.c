...
/*
 * FIREFLY VENDORED COPY of espressif/esp_lcd_touch_spd2010 v2.0.1
 * (esp-iot-solution commit c29b0042d4011d907f78a484f12ed187e65cbeab).
 *
 * WHY VENDORED (do not "clean up" back to the managed component):
 * The stock driver reads registers with `esp_lcd_panel_io_rx_param(tp->io,
 * 0, buf, len)` — i.e. lcd_cmd = 0, not -1. Under ESP-IDF 5.3.x's NEW
 * i2c_master driver, esp_lcd's i2c panel IO (esp_lcd_panel_io_i2c_v2.c)
 * treats any lcd_cmd != -1 as "send a command first", so it calls
 * i2c_master_transmit_receive() with a ZERO-length write phase (control
 * phase is disabled and lcd_cmd_bits = 0 in ESP_LCD_TOUCH_IO_I2C_SPD2010_
 * CONFIG). i2c_master_transmit_receive() rejects write_size == 0 with
 * ESP_ERR_INVALID_ARG ("i2c transmit buffer or size invalid"), so
 * read_fw_version() fails and esp_lcd_touch_new_i2c_spd2010() aborts —
 * the exact STAGE-3 failure seen on the bench. The SPD2010 protocol is a
 * write (sets the register pointer) followed by a SEPARATE pure read; the
 * correct panel-IO idiom for a pure read is lcd_cmd = -1, which routes to
...
/* FIREFLY PATCH: lcd_cmd -1 (was 0) -> a PURE i2c read (i2c_master_receive),
 * no zero-length write phase. See the file-header note. The preceding
 * i2c_write() already latched the register pointer; this reads from it. */
