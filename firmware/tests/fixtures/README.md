...
| `next` | object or omitted | omitted (`ff_app_next_t.valid = false`) | Populated for `state: "live"` and `state: "mixed"`. Omit entirely (not `null`) when there's no upcoming starred set — presence of the `next` object sets `valid = true`. |
...
| `now_mixed.json` | `mixed` | Code review round 2's fix in action: one known-time row (Excision) and one known-time starred-next (TYNAN, "IN 58 MIN") render normally under a "KNOWN SO FAR" section, WHILE five still-unknown-time sets (NGHTMRE, Borgore, Levity, Doctor P, Hairitage — the rest of the real Lost Lands day-1 lineup) stay visible in a "STILL TBD" list under the same banner `now_tbd.json` uses, rather than silently vanishing the moment the day stopped being all-null. |
