...
| `close_leg` | string enum: `none`\|`distance`\|`rssi` | `none` | **2026-09-16 amendment (close-range-honest-distance).** Which leg of `ff_crew_close_range_leg()` fired when `mode: "close"` — `none` is also the enum's zero value, so an absent key defaults honestly. `distance`: `dist_str` is the fixed `FF_CREW_CLOSE_RANGE_M` threshold ("30 m"/"98 ft"), never a measured point distance (two "precise" consumer GPS fixes a foot apart can disagree by 2-15 m — see `docs/specs/S06-radar-face.md`'s 2026-09-16 amendment); renders "WITHIN 30 M". `rssi`: `dist_str` is `""` (no coordinate involved at all — S29's "signal is never distance"); renders "NEARBY". See `radar_close.json`/`radar_close_rssi.json` below for the golden pair. |
...
member but still said "NO LOCATION - RADIO ONLY / Looking for X"
(wording per A02/PR #303, 2026-09-13 — was "NO FIX - RADIO ONLY"),
which is false on both counts. Renders name+distance as in `live`, no
arrow, an amber "NO COMPASS" chip, and a bare compass-letter hint (e.g.
"S") computed from `bearing_deg` via `ff_geo_compass_point` (16-point
names, `core/include/ff_geo.h`) — the 2026-09-13 pass dropped the older
mono-styled "BEARING <bearing_deg> · <compass point>" framing.
...
`you` section at all, so the arrow is hidden and the "NO LOCATION"
chip (was "NO FIX") renders instead). All four are synthetic (no mockup artboards in-tree
...
| `radar_close.json` | `close` | `30 m` (`close_leg: "distance"`) | `3 SEC` | close-range predicate tripped BY DISTANCE; `arrow_valid: false` per S06 ("false in CLOSE/NOFIX/NOSEL"). **2026-09-16 amendment (close-range-honest-distance):** `dist_str` used to be the measured `15 m` and rendered `~15 m` — now it is the fixed `FF_CREW_CLOSE_RANGE_M` threshold (never the measured value, which two "precise" GPS fixes a foot apart can disagree on by 2-15 m) and renders "WITHIN 30 M". See `radar_close_rssi.json` below for the other leg |
| `radar_close_rssi.json` | `close` | `""` (`close_leg: "rssi"`) | `""` | **2026-09-16 amendment (close-range-honest-distance).** The OTHER way CLOSE fires: no usable position at all (`dist_str`/`age_str` both `""`, mirroring `radar_nofix.json`'s "distance unknown" convention), but a fresh, strong DIRECT RSSI sample trips the RSSI leg — renders "NEARBY" (never a distance number; S29's "signal is never distance" rule) instead of `radar_close.json`'s "WITHIN 30 M". Supersedes this fixture family's only prior route to "NEARBY", which was issue #47's imprecise-position special case (`dist_imprecise: true` forcing the DISTANCE leg off) — that path still reaches `close_leg: "rssi"` today (imprecision forces the distance leg unusable), but this fixture shows the plainer, more common case: no position on file for this member at all |
...
| `radar_nohdg.json` | `nohdg` | `492 ft` | `8 SEC` | 2026-09-05 amendment — my heading is unknown (`bearing_valid: true`, `arrow_valid: false`); name+distance stack as in `live`, amber "NO COMPASS" chip in place of the arrow, a bare compass-letter hint ("S") below the distance (`bearing_deg: 180`; the 2026-09-13 plain-language pass dropped the older mono "BEARING 180° · S" framing). Fresh fix (`stale: false`) so no rim tint — see `radar_nohdg_stale.json` for the tinted case |
...
| `radar_long_name.json` | `live` | `320 m` | `8 SEC` | 2026-09-06 crew long names — `name: "Bartholomew Montgomery-Fitzgerald"`, deliberately past `radar_build_name_label`'s reserved width for this mode: pins the LVGL `LONG_MODE_DOTS` ellipsis ("Bartholomew Montg…") rather than a wrap or an off-glass overflow |
